"object" == typeof navigator && function (e, t) {
    ...
    const xe = {
            setup() {
                ...
                const e = xe.getTracks.call(this, !0), {active: t, language: i, meta: s, currentTrackNode: n} = this.captions,
    ...
    class Ve {
        ...
        handleKey(e) {
            const {player: t} = this, {elements: i} = t, {key: s, type: n, altKey: a, ctrlKey: l, metaKey: r, shiftKey: o} = e,
                c = "keydown" === n, u = c && s === this.lastKey;
            if (a || l || r || o) return;
            if (!s) return;
            if (c) {
...
function videoLinksInit(videoItemSelector) {
    if (!videoItemSelector)
        videoItemSelector = '.video-item__link';

    jQuery(videoItemSelector + '.external-video-file').on("click", function (e) {
        ...
        jQuery.magnificPopup.open({
            ...
            items: {
                src:
                    ...
                    '<div class="video-plyr" data-plyr-provider="' + videoCheck(videoSrc) + '" data-plyr-embed-id="' + videoSrc + '" data-plyr-config=\'{ "autoplay": "true", "iconUrl": "/resources/img/sprite.svg"  }\'></div>' +
                    '</div>'
            },
            callbacks: {
                open: function () {
                    window.players.push(new Plyr('.video-plyr'));

                    var topScroll = jQuery(window).scrollTop();
                    this.wrap.css('top', topScroll);
                    this.bgOverlay.css('top', topScroll);
    ...
    jQuery(videoItemSelector + '.local-video-file').on("click", function (e) {
        e.preventDefault();
        ...
        var video_items_plyr_attr;
        if (!/iPad|iPhone|iPod/.test(navigator.userAgent)) {
            video_items_plyr_attr = 'data-plyr-config=\'{ "autoplay": "true", "iconUrl": "/resources/img/sprite.svg"  }\'';
        }
        jQuery.magnificPopup.open({
            type: 'inline',
            mainClass: 'mfp-fade',
            ...
            items: {
                src:
                    '<div class="video-popup">' +
                    '<video class="video-popup__player" ' + videoPoster + ' ' + video_items_plyr_attr + '>' +
            ...
            callbacks: {
                open: function () {
                    if (!/iPad|iPhone|iPod/.test(navigator.userAgent)) {
                        window.players.push(new Plyr('video[data-plyr-config]'));

                    } else {
                        document.querySelector('.video-popup__player').play();
                    }
...
window.players.push(new Plyr('#player'));

jQuery('.external-player .plyr__control--overlaid').on('click', function () {
    window.players.push(new Plyr(this.parentElement));
});